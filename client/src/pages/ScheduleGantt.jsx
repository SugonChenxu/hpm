/**
 * ScheduleGantt —— 单条排期计划的甘特图编辑器
 *
 * 从 QuickSchedulePage 抽取，封装「轨道 + 进度条/箭头线 + 关键节点 + 竖虚线」的
 * 渲染与全部编辑交互。父组件（多排期折叠列表）传入完整 detail 与 onChange 回调，
 * 本组件只在自身 schedule 上操作，便于在同一页面内并行挂载多条排期。
 */
...
export const MIN_SCHEDULE_MONTHS = 6;
export const MIN_MONTH_WIDTH = 24; // 月份过多时兜底最小宽度（避免标签完全挤压）
...
/**
 * 单条排期甘特图编辑器
 * @param {object} schedule 完整 detail（含 tracks / vlines）
...
export default function ScheduleGantt({ schedule, onChange }) {
  ...
  const handleDrop = async (targetTrackId) => {
    ...
    onChange({ ...schedule, tracks: reordered });
