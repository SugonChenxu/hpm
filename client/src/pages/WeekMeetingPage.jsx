...
import { ChevronLeft, ChevronRight, Add, DeleteOutline, EventNote } from "@mui/icons-material";
  ...
  return (
    <Box>
      ...
      <Box sx={{
        ...
        {WEEKDAYS.map((d) => (
          <Box key={d} sx={{ p: 0.5, textAlign: "center", fontWeight: 600, bgcolor: "grey.100", borderBottom: "1px solid", borderColor: "divider", borderLeft: "1px solid", borderColor: "divider" }}>
            {d}
          </Box>
        ))}
        {/* 时间行 */}
        {TIME_SLOTS.slice(0, -1).map((time, rowIdx) => (
          <Row key={time} time={time} rowIdx={rowIdx} meetingsByDay={meetingsByDay} onDelete={handleDelete} />
        ))}
      </Box>

      {/* 输出物清单 */}
      <Typography variant="h6" fontWeight={700} sx={{ mt: 3, mb: 1.5 }}>每日输出物清单</Typography>
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(2, 1fr)", md: "repeat(4, 1fr)" }, gap: 2 }}>
        {WEEKDAYS.map((d) => (
          <Box key={d}>
            <Typography variant="caption" color="text.secondary" sx={{ mb: 0.5, display: "block" }}>{d}</Typography>
            <TextField
              multiline
              minRows={2}
              maxRows={4}
...
/** 单行(时间槽) — 7天格子 + 叠加会议卡片 */
function Row({ time, rowIdx, meetingsByDay, onDelete }) {
  return (
    <>
      {/* 时间标签 */}
      <Box sx={{ p: 0.5, textAlign: "center", color: "text.secondary", borderBottom: "1px solid", borderColor: "divider", fontSize: "0.7rem" }}>
        {time}
      </Box>
      {/* 7天格子 */}
      {WEEKDAYS.map((day) => {
        ...
        return (
          <Box key={day} sx={{ borderBottom: "1px solid", borderLeft: "1px solid", borderColor: "divider", minHeight: 28, position: "relative", p: 0.25 }}>
            {meetings.map((m) => {
              ...
              return (
                <Tooltip key={m.id || m.title} title={m.title} arrow>
                  <Box sx={{
                    bgcolor: isProject ? `${color}15` : `${color}20`,
                    borderLeft: `3px solid ${color}`,
                    borderRadius: 0.5,
                    p: 0.5,
                    height: span ? `${span * 28 - 2}px` : "auto",
                    overflow: "hidden",
                    ...
                    <Box sx={{ display: "flex", alignItems: "flex-start", gap: 0.25 }}>
                      <Typography sx={{ fontSize: "0.68rem", fontWeight: 600, flex: 1, lineHeight: 1.2, color: color, wordBreak: "break-all" }}>
                        {m.title}
                      </Typography>
                      {!isProject && (
                        <IconButton size="small" sx={{ p: 0, opacity: 0.5, "&:hover": { opacity: 1 } }}
