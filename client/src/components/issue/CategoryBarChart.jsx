...
export default function CategoryBarChart({ data = [], loading = false }) {
  ...
  const chartData = filtered.map((d, i) => ({
    ...
    label: `${d.category} DI${Math.round(d.count * 100) / 100}`,
      ...
      <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5, mt: 1, justifyContent: "center" }}>
        {chartData.map((d, i) => (
          <Box key={i} sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
            <Box sx={{ width: 12, height: 12, borderRadius: 0.5, bgcolor: COLORS[i % COLORS.length] }} />
            <Typography variant="caption">{d.category} DI{d.di}</Typography>
          </Box>
        ))}
      </Box>
    </Box>
