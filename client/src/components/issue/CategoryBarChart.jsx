...
export default function CategoryBarChart({ data = [], loading = false }) {
  ...
  if (loading) return <Box sx={{ mb: 3 }}><Typography variant="h6" gutterBottom>缺陷分布（按分类）</Typography><Skeleton variant="rectangular" height={280} sx={{ borderRadius: 1 }} /></Box>;
    ...
    <Box sx={{ mb: 3 }}>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
          ...
          <Tooltip formatter={(value) => [`DI ${value}`, ""]} labelFormatter={(label) => `分类: ${label}`} />
          <Legend />
