...
- **用户**: 多用户（局域网内网网页集中部署，各自独立管理；2026-07-20 起由单用户扩展）
...
| M9 | 库存管理（PLM 研发库房对接） | docs/modules/09-库存管理模块-PRD.md |
| M10 | 快速笔记（富文本随手记） | docs/modules/10-快速笔记模块-PRD.md |
...
- 多用户改造细化决策（2026-07-20 收口）：
  - 认证方式：**Session**（服务器会话，内网小团队简单够用；非 JWT）
  - 账号开通：**你手动逐个添加**（不开放注册；提供建账号脚本，每个同事单独建）
  - 服务器机器：**就用你本机办公电脑常开**（不另购/另找独立服务器；数据先放本机）
  - 会话时效：**尽量长**（30 天或持久「记住我」，减少同事重复登录）
  - 现有数据迁移：先为本人建专属账号（如 chenxu），将 D:/HPM 现有全量数据 migrate 绑定到该 owner_id，避免登录后空白
  - 演进路径：**先本机试用 → 后整包迁移公司内网服务器**（SQLite 单文件拷贝即迁移，代码无需改）
