...
| 系统字段 | 候选列名（中/英） |
|----------|------------------|
| 任务名称 | 任务、任务名、名称、事项、工作项 / name、task、title |
| 任务类型 | 任务类型、类型 / tasktype、type |
| 开始 | 开始、开始日期、起始、计划开始 / start、startdate |
| 结束 | 结束、结束日期、完成、截止、计划结束 / end、enddate |
| 工期 | 工期、天数 / duration |
| 前置 | 前置、前置任务、依赖 / predecessor、depends |
| 备注 | 备注、说明 / notes、remark |
| 层级 | 层级、缩进、级别 / level、indent |
| 序号 | 序号、seq、no、行号（自动重排，忽略） |

**阶段 / 普通 / 节点任务自动区分规则**（三级 fallback）：
1. **优先**「任务类型」列的值：阶段 / 里程碑 / 节点 → 对应 `阶段任务` / `节点任务` / `普通任务`（别名兼容 phase/stage/milestone/task 等）；
