...
> **⚠️ 浏览页链接 vs 下载链接**：腾讯文档「浏览页链接」（`docs.qq.com/sheet/...`，即浏览器地址栏里直接打开的链接）返回的是**网页 HTML 而非 Excel**——后端 `fetch` 拿不到表格数据会直接 400 报错并提示。请改用以下任一方式：
> 1. **下载链接**：在腾讯文档中点「下载 / 导出为 Excel」→ 复制下载链接（形如 `*.xlsx` 直链）粘贴到导入框；或
> 2. **本地文件**：直接「下载为 Excel」后用「导入 Excel」选择本地文件。
> 后端已对响应做内容类型校验（检查 `content-type` 与文件头 `PK\x03\x04`），命中浏览页会返回清晰指引而非崩溃。

**多级分组（大阶段 / 小阶段 / 任务）**：当表头出现 ≥2 个分组列（命中「大阶段 / 中阶段 / 小阶段 / 阶段 / 分组 / group / phase / stage」）时进入多级分组模式，支持如 `大阶段 | 小阶段 | 任务 | 开始 | 结束 | 工期` 的三级结构：
- 按 `lastGroups[]` 跟踪各级当前值，**空白单元格默认沿用上一级**（兼容合并单元格 / 续行写法），仅当更高级本行变化时重置；
- 为每个级的非空值自动合成「**阶段任务**」父节点（如 大阶段「详设」下挂 小阶段「板卡_主板 / 散热 / 结构…」），叶子任务 `indent = 最高组级别 + 1`；
- 后端 `insertScheduleTasks` 用 indent 栈还原 `parent_id`，大阶段时间由 `recalcPhaseAggregation` 聚合其全部后代（不会误混入其它大阶段的节点）。

**中文无年日期推断**：支持 `X月X日` / `X月X号` 写法（如 `12月23日`、`1月6号`）。映射器收集所有仅含月日的单元格，按「**首个日期月为起点月**」推断年份——月份 ≥ 起点月记为起始年 Y0，< 起点月记为 Y0+1，从而在跨年边界（如 12月→次年1月）正确切换且不逐行累加年份。已含年份的 `2025/12/23`、`2025-12-23` 仍按原样解析。

**导入反馈**：前端在导入对话框内联展示错误 `Alert`（如浏览页链接提示、解析失败原因）；成功时提示「已导入 N 条计划」。解析失败（无任务列 / 空文件）给出明确错误。
