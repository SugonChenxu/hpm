...
| 已完成栏 | ✅ | 按 completed_at 倒序排列，灰化+删除线，opacity 0.6，✓ M/D 完成时间标签；**默认仅展示最近完成的 6 条，超出时底部出现「展开全部 N 条 ▾」折叠开关**（详见 2.5）；**待办栏无条数上限**（全量渲染 + 容器内滚动） |
| 撤销完成 | ✅ | 悬浮回退箭头，点击移回待办栏 |
| 删除任务 | ✅ | 悬浮删除按钮 + 二次确认弹窗 |
...
| 撤销待办 | ✅ | 悬浮回退箭头 ↺ |
| 删除 | ✅ | 悬浮删除按钮 ✕ + 二次确认 |
| 单行水平对齐 | ✅ | 已完成栏每行：优先级状态灯、任务标题、✓ M/D 完成时间、📋 子任务标识、↺ 撤销、✕ 删除 **全部位于同一水平行**（flex `alignItems:center`），标题超长省略号截断（已完成栏 UI 标准已落地） |
| 条数上限与折叠 | ✅ | 已完成栏默认仅显示最近完成的 6 条（按 completed_at 倒序）；超出 6 条时底部显示「展开全部 N 条 ▾」，点击展开全部 / 「收起 ▴」折叠回 6 条；待办栏不限制条数 |
...
- 子任务完成不影响父任务状态（按设计决策）
- 看板卡片拖拽顺序已通过 `PUT /api/tasks/:id/reorder` 持久化 `sort_order`（2026-07-20 校订：原「未持久化」已失效）
