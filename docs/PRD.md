...
| **M8** | 用户管理模块 | 账号与权限网页化管理——新增/改密/重置/删除/分配角色，替代命令行建账号；owner/admin/member 三级权限 | — | 已落地（2026-07-21） |
...
| M8 用户管理模块 | `docs/modules/08-用户管理模块-PRD.md` | 已落地（网页加用户/改密码/重置/删除） |
...
> **2026-07-21 PRD 同步**：将近期实装的功能变更同步至对应模块 PRD——① M1：项目概览卡片新增「故障概览」区块（DI/故障总数/解决率 + DI 趋势 sparkline + 未解决分类饼图，关联 M3），接口表新增 `GET /api/projects/:id/faults`；② M2：已完成栏改为单行水平对齐（状态灯/标题/完成时间/撤销/删除同行）+ 上限 6 条折叠，待办栏无条数上限；③ M3：缺陷分布柱状图/饼图统一 10 色高区分度调色板（去重紫色撞色），新增未解决分类分布口径。
