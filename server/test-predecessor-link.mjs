...
try {
  ...
  // ===== 场景1：A 有单前置 B，B.end 晚于 A 新开始 → B.end = A.start-1，B.start 不变，工期重算 =====
  {
    ...
    const res = linkPredecessorsToStart(pid, A, "2026-08-15"); // A 提前到 8-15
    const b = q("SELECT * FROM schedule_tasks WHERE id=?", B);
    check("场景1 触发联动", res.linked.length === 1 && res.linked[0] === B);
  ...
  // ===== 场景2：B.end 早于 A 新开始 → B 不变化 =====
  {
    ...
    const res = linkPredecessorsToStart(pid, A, "2026-08-01"); // A 不变（或推迟）
    const b = q("SELECT * FROM schedule_tasks WHERE id=?", B);
    check("场景2 不触发联动", res.linked.length === 0);
  ...
  // ===== 场景3：多前置 B/C/D，B.end>=A.start、C.end<A.start、D.end>=A.start → B/D 联动，C 不动 =====
  {
    ...
    const res = linkPredecessorsToStart(pid, A, "2026-08-20"); // A 提前到 8-20
    const b = q("SELECT * FROM schedule_tasks WHERE id=?", B);
    const c = q("SELECT * FROM schedule_tasks WHERE id=?", C);
    const d = q("SELECT * FROM schedule_tasks WHERE id=?", D);
    check("场景3 联动 B 和 D", res.linked.length === 2 && res.linked.includes(B) && res.linked.includes(D) && !res.linked.includes(C));
  ...
  // ===== 场景4：前置是阶段任务 → 跳过 =====
  {
    ...
    const res = linkPredecessorsToStart(pid, A, "2026-08-05");
    const ph = q("SELECT * FROM schedule_tasks WHERE id=?", PH);
    check("场景4 阶段任务前置跳过", res.linked.length === 0 && ph.planned_end === null);
  }
  ...
  // ===== 场景5：前置是节点任务 → 跳过 =====
  {
    ...
    const res = linkPredecessorsToStart(pid, A, "2026-08-15");
    const n = q("SELECT * FROM schedule_tasks WHERE id=?", N);
    check("场景5 节点任务前置跳过", res.linked.length === 0 && n.planned_end === "2026-08-10");
  }
  ...
  // ===== 场景6：前置锁定任务 → 跳过 =====
  {
    ...
    const res = linkPredecessorsToStart(pid, A, "2026-08-10");
    const l = q("SELECT * FROM schedule_tasks WHERE id=?", L);
    check("场景6 锁定前置跳过", res.linked.length === 0 && l.planned_end === "2026-08-20");
  }
  ...
  // ===== 场景7：压缩后 结束<开始 的防御（B 整体晚于 A 的新开始，脏数据场景）=====
  {
    ...
    const res = linkPredecessorsToStart(pid, A, "2026-08-15"); // 压缩 B.end=8-14 < B.start=8-25 → 跳过
    const b = q("SELECT * FROM schedule_tasks WHERE id=?", B);
    check("场景7 防御跳过", res.linked.length === 0 && b.planned_end === "2026-09-05");
    check("场景7 返回 warning 提示", res.warnings.length === 1 && res.warnings[0].includes("B7"), res.warnings[0] || "无");
  }
  ...
  // ===== 场景8：A 无前置 → 无联动 =====
  {
    ...
    const res = linkPredecessorsToStart(pid, A, "2026-08-05");
    check("场景8 无前置不触发", res.linked.length === 0);
  }
  ...
  // ===== 场景9：A 后移，前置 B 原本紧贴（B.end = 旧A.start - 1）→ B 结束顺延 = 新A.start - 1 =====
  {
    ...
    const res = linkPredecessorsToStart(pid, A, "2026-09-01", "2026-08-15"); // A 后移到 9-01
    const b = q("SELECT * FROM schedule_tasks WHERE id=?", B);
    check("场景9 触发顺延", res.linked.length === 1 && res.linked[0] === B);
  ...
  // ===== 场景10：A 后移，前置 B 远早于（B.end < 旧A.start - 1）→ 不动 =====
  {
    ...
    const res = linkPredecessorsToStart(pid, A, "2026-09-01", "2026-08-15");
    const b = q("SELECT * FROM schedule_tasks WHERE id=?", B);
    check("场景10 远早不联动", res.linked.length === 0);
  ...
  // ===== 场景11：A 后移但前置 B 仍重叠（B.end >= 新A.start）→ 压缩到 新A.start - 1 =====
  {
    ...
    const res = linkPredecessorsToStart(pid, A, "2026-09-05", "2026-08-15"); // A 后移到 9-05
    const b = q("SELECT * FROM schedule_tasks WHERE id=?", B);
    check("场景11 重叠压缩", res.linked.length === 1 && b.planned_end === "2026-09-04", `${b.planned_end}`);
  ...
  // ===== 场景12：A 前移，原本紧贴的 B 被压缩（回归上轮行为）=====
  {
    ...
    const res = linkPredecessorsToStart(pid, A, "2026-08-10", "2026-08-15"); // A 前移到 8-10
    const b = q("SELECT * FROM schedule_tasks WHERE id=?", B);
    check("场景12 前移压缩 B.end=8-09", res.linked.length === 1 && b.planned_end === "2026-08-09", b.planned_end);
    check("场景12 B.工期=9", b.duration_days === 9, String(b.duration_days));
  }

  // ===== 场景13：日期带时间部分（脏数据）时紧贴判断仍生效（规范化比较）=====
  {
    const B = mk("B13", "普通任务", "2026-08-01", "2026-08-14 00:00:00", 14, []); // end 带时间
    const A = mk("A13", "普通任务", "2026-08-15", "2026-08-26", 12, [B]);
    const res = linkPredecessorsToStart(pid, A, "2026-09-01", "2026-08-15"); // A 后移
    const b = q("SELECT * FROM schedule_tasks WHERE id=?", B);
    check("场景13 规范化比较触发顺延", res.linked.length === 1 && b.planned_end === "2026-08-31", `${b.planned_end}`);
  }

  // ===== 场景14：只改结束时间（planned_end）不触发联动 =====
  {
    const B = mk("B14", "普通任务", "2026-08-01", "2026-08-20", 20, []);
    const A = mk("A14", "普通任务", "2026-08-25", "2026-09-05", 12, [B]);
    // 模拟 PUT 只改 end 的场景：updates 里无 planned_start → 不联动
    const res = linkPredecessorsToStart(pid, A, "2026-08-25", "2026-08-25"); // newStart 与 oldStart 相同（=只改 end 时不调用的等价判断）
    const b = q("SELECT * FROM schedule_tasks WHERE id=?", B);
    check("场景14 值未变不联动", res.linked.length === 0 && b.planned_end === "2026-08-20");
  }
