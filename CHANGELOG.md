...
## 2026-08-31 — 【快速排期】PPT 进度条移除白色斜纹（彻底纯色）
- 模块：`server/src/quick-schedule-pptx.js`
- 缺陷：进度条上的「白色斜纹」（transparency 55 的半透明白条）盖在色块上形成白色遮盖，用户反馈仍有白色阴影。
- 修复：删除 `addBarShape` 内斜纹循环与形状，进度条改为纯色矩形（内嵌文字），无任何白色覆盖物。
- 保留：卡片毛玻璃底框（半透明白，预期效果）、节点黑阴影、卡片黑阴影；箭头线纯色。
- 验证：mock 生成 pptx 校验 `ALPHA_WHITE_FILL=1`（仅为卡片毛玻璃）、`OUTER_SHDW=2`（卡片+节点黑）、SP_COUNT=18 无斜纹；语法通过；forge 重启 online。
...
## 2026-08-31 — 【快速排期】PPT 阴影区分纯色 / 黑阴影 / 白阴影
- 模块：`server/src/quick-schedule-pptx.js`
- 新增 `isDarkColor()` 亮度判断 + `SHADOW_WHITE` 白色阴影预设。
- 进度条/关键节点按填充色自动选阴影：深色填充→白色阴影（黑阴影不可见），浅色填充→黑色阴影；卡片保持黑阴影（白底）；箭头线、白色斜纹保持纯色无阴影。
- 验证：mock 深/浅色排期生成 pptx，slide1.xml 校验 5 阴影=黑3+白2，颜色感知正确；语法通过；forge 重启 online。
...
- 前端：折叠卡片头部新增复制按钮（⧉），复制后新副本置顶并自动展开
- `client.js` 新增 `quickSchedules.duplicate(id)`

## 2026-08-21 — 【快速排期】打开折叠卡片边界限制，修复笔记本小屏轨道显示不全

- **缺陷修复① 卡片裁剪**：折叠卡片 `overflow: "hidden"` 移除。此前卡片圆角裁剪 + 固定页面高度的内层滚动，使展开后底部轨道在小屏（笔记本）被裁掉或滚不到；现卡片不再裁剪内容。
- **缺陷修复② 甘特纵向边界**：`ScheduleGantt` 根容器去掉 `flex:1/minHeight:0`（不再尝试填充受限高度）；甘特区滚动容器去掉 `flex:1/overflowY:auto/minHeight:0`，改为**纵向自然撑开、仅保留 `overflowX:auto` 横向滚动**。展开后全部轨道完整渲染，由外层列表滚动查看，不再被内层视口截断。
- **参数变动** 无；后端接口无改动。
- **验证** `vite build` 通过；生产 `forge` 重启在线，新 bundle `index-BrKK87tj.js` 生效。

## 2026-08-20 — 【快速排期】修复小屏折叠页无法正常下拉/查看（移动端 100vh + 甘特横向滚动）

- **缺陷修复① 移动端视口高度**：页面固定高度由 `calc(100vh - 64px)` 改为 `calc(100dvh - 64px)`。`100vh` 在手机浏览器中高于可见视口（被地址栏撑大），导致内部滚动容器底部落到屏幕外、下拉滚不到底；`dvh`（dynamic viewport height）随浏览器 UI 动态收缩，内容可达。
- **缺陷修复② 甘特横向滚动**：甘特区滚动容器 `overflowX` 由 `hidden` 改为 `auto`，小屏下时间轴可横向拖拽/滚动查看后续月份（此前被裁剪）。
- **缺陷修复③ 移动端滚动手感**：列表与甘特滚动容器均加 `WebkitOverflowScrolling: "touch"`，开启惯性滚动。
- **参数变动** 无；后端接口无改动。
- **验证** `vite build` 通过；生产 `forge` 重启在线，新 bundle `index-BmkSoz_t.js` 生效。

## 2026-08-20 — 【快速排期】折叠列表标题简化：单标题 + 删除冗余信息

- **界面调整① 标题只显示一次**：标题仅保留在**折叠/展开标题行**；甘特图工具栏内不再重复显示排期标题（删除该「第二行」标题），避免同一卡片内出现两处标题。
- **界面调整② 标题行只显示标题**：折叠标题行移除「时间范围」与「轨道数」两个 Chip，只保留 展开箭头 + 标题 + 删除按钮。
- **新增功能 标题重命名**：标题行点击标题文字即可就地重命名（回车/失焦保存，Esc 取消），替代原甘特工具栏内的标题编辑，保留改名能力。
- **参数变动** 无；后端接口无改动。
- **验证** `vite build` 通过（12242 模块，0 报错）；生产 `forge` 重启在线，新 bundle `index-BdWERJr0.js` 生效。

## 2026-08-20 — 【快速排期】多项目排期：折叠/展开列表 + 删除排期

- **新增功能① 多项目排期（折叠列表）**：快速排期由「单条计划」升级为「多条计划并列管理」。页面加载全部排期（`GET /quick-schedules` 列表 + 逐条 `GET /quick-schedules/:id` 详情），以**手风琴卡片**呈现；每条计划头部显示标题、时间范围、轨道数，左侧按序循环强调色（便于区分不同项目）。
- **新增功能② 折叠/展开**：点击卡片头部或箭头（▸/▾）即可**隐藏/显示该排期计划的甘特图**；支持「全部展开 / 全部折叠」一键切换；默认展开最近编辑的一条。折叠态不挂载甘特组件（省去测量与渲染开销）。
- **新增功能③ 删除排期**：每条计划头部「✕」按钮 → 二次确认弹窗 → 调 `DELETE /quick-schedules/:id`。**级联清理**：DB 表 `quick_schedules` ↔ 子表（`tracks/bars/milestones/vlines`）已建 `ON DELETE CASCADE` 且 `PRAGMA foreign_keys=ON`，删除父表即自动清理全部子数据，无残留孤儿行。
- **重构** 抽取 `client/src/pages/ScheduleGantt.jsx`（默认导出 `ScheduleGantt`）：将甘特图渲染与全部编辑交互（拖拽进度条/节点、轨道增删改、参照线、导出 PPT/Excel、标题与时间段编辑）从页面抽离，改为接收 `schedule` + `onChange(detail | (prev)=>next)`，父组件 `QuickSchedulePage` 仅负责列表编排与折叠/删除，从而支持同一页面并行挂载多条排期且各自独立编辑。
- **接口调整** 无新增/变更；复用既有 list/create/get/update/remove 及 tracks/bars/milestones/vlines 子资源接口。
- **参数变动** 无。
- **验证** `vite build` 通过（12242 模块，0 报错）；生产 `forge` 重启在线，根路径 200、API 鉴权 401（符合多用户会话模型）。

## 2026-08-19 — 【快速排期】「导出 Excel」V4：全面对齐 PPT 导出（配色/底色/视觉 + 多项缺陷）

- **配色/底色/视觉参考 PPT 导出**（`quick-schedule-pptx.js` 同源）：标题深色 #1F2937 无底色；季度行红底 #A94442 白字；月份行浅红底 #D9A6A5 白字；轨道区月带 **白/EEF2F7 交替**；轨道名 = 浅色底纹（lighten 12% 混合）+ 深字 #374151 + 左侧粗色块边框；进度条 8pt 白字 + 白色细边；节点符号 0.16"≈15px、**文字在符号下方居中**（7.5pt，色 = ms.text_color||黑）；轨道行高 36pt（PPT ROW_H=0.5"）。
- **缺陷修复① 标题黑字看不见**（根因）：ExcelJS 传 6 位色值（如 `"1F2937"`）会写成 `color auto="1"`（自动=黑）→ 深底上黑字不可见。修复：全部单元格字体色显式 `{ argb: argb("FF…") }`。核验 styles.xml：FF1F2937/FFFFFFFF/FF374151/FF6B7280 均正确写入。
- **缺陷修复② 删除第二行**：移除副标题/统计行，布局 = 标题(1) / 季度(2) / 月份(3) / 轨道(4..10)，冻结 ySplit=3。
- **缺陷修复③ A3/A4 角格**：「季度」「月份」角格文字移除（留空仅带底色）。
- **缺陷修复④ 轨道线长度**：箭头线右端对齐**最后一个月份右侧**（colTo=c2+1, colOffTo=0），核验 7 条全部 colTo=16/colOffTo=0。
- **缺陷修复⑤ 节点文字位置**：标签从「符号右侧」改为「符号下方居中」（PPT 式），核验 18/18 在符号下方。
- **验证** 48 形状（7 箭头线 + 3 进度条白边 + 18 节点 + 18 下方标签 + 2 虚线）、pics=0、线条 0 斜、XML（含 styles.xml）全良构、生产重启在线。
- **产出** `D:/tmp/项目规划_快速排期_月度示意_V4.xlsx`；页面「导出 Excel」已生效。

## 2026-08-19 — 【快速排期】「导出 Excel」V3：线条拉直 + 配色简化 + 节点/进度条按 Forge 比例定位

- **缺陷修复① 线条不直**：轨道箭头线/参照虚线此前是「盒对角」线（from/to 的 rowOff 或 colOff 不同 → 有微小斜度）。修复：箭头线 `rowOffFrom == rowOffTo`（绝对水平）、参照线 `colOffFrom == colOffTo`（绝对垂直）。核验：7 箭头 + 2 虚线 **not-straight=0**。
- **缺陷修复② 配色/可读性**：去掉轨道区月度交替底色带（统一白色）；月份表头改单色浅底（E8EEF7）深字；节点名称框从「居中在 2 个月宽盒子中央」改为**紧随符号左侧对齐**（`algn="l"`），避免文字漂浮错位难读；文字统一深灰 #374151 保证对比度；进度条/节点颜色直接输出数据色。
- **缺陷修复③ 位置按 Forge 比例推算**：复刻 `dateToPixels = 月序号×月宽 + (当月已过天数/当月总天数)×月宽`——节点符号中心对准日期点、进度条左=开始日位置、右=结束日位置、参照线=日期点竖线（此前是整月居中/整月占满）。核验：18 节点中心 px 逐一吻合公式（如 4-15→44.8px、1-26→77.4px、8-31→92.9px）。
- **验证** 48 形状结构不变（7 箭头线 + 3 进度条 + 18 节点 + 18 名称框 + 2 虚线）、pics=0、XML 全良构、生产重启在线。
- **产出** `D:/tmp/项目规划_快速排期_月度示意_V3.xlsx`；页面「导出 Excel」已生效。

## 2026-08-19 — 【快速排期】「导出 Excel」月度版微调：表格宽度 + 节点尺寸对齐 Forge

- **需求** 参考 Forge 前端样式调整：① 表格宽度（月列过宽 159px → 收窄）；② 关键节点形状大小（原节点菱形实际占 1.44×月列 ≈ 229px，严重偏大）；③ 保持美观。
- **对齐 Forge 常量**（`QuickSchedulePage.jsx`）：轨道列 `LABEL_WIDTH=160px` → 导出列宽 22 字符（≈160px）；月宽自适应约 80~115px → 导出月列 13 字符（≈96px）；节点符号 `size=18` → 节点形状固定 18px（square 按 Forge 宽 0.58×s）；行高 64px → 轨道行 32pt；进度条约 1/5 行高 → 条高 ≈ 52% 行高（保证文字可读）。
- **修复** 节点锚点从「跨格偏移」（colTo=c+1 + 28%~72% 偏移，实际宽度 = 1.44×列宽）改为「同格锚点」（colFrom=colTo=c，仅用 colOff 定位，宽度精确 = 18px）；节点名称框收窄为 c+1..c+2。
- **验证** 48 形状结构不变（7 箭头线 + 3 进度条 + 18 节点 + 18 名称框 + 2 虚线）；**全部 18 个节点宽度精确 18px（min=max=18）**；XML 全良构；生产重启在线。
- **产出** `D:/tmp/项目规划_快速排期_月度示意_V2.xlsx`（预览路径已同步）；页面「导出 Excel」已生效。

## 2026-08-19 — 【快速排期】「导出 Excel」升级为月度示意版（一格一月 + 原生形状 + 美化）

- **需求** 用户要求：横向表头=季度+月份（**一格一月，不逐日还原**）、纵向=轨道名称、用形状代表关键节点与进度条、最大程度还原甘特效果并**提升美观度**；载体选定**本地 Excel**（原生形状可自由编辑，腾讯文档 API 无法插入原生形状）。
- **重构** `server/src/quick-schedule-xlsx.js`：
  - 布局：月度网格（2026-08..2027-10 共 15 个月列，宽 22 字符）；标题深蓝底白字 + 副标题统计行；季度行按连续区间合并（2026Q3..2027Q4，蓝系）；月份行一格一月交替底色；轨道区月度交替底色带；轨道标签彩色块；底部图例行；冻结表头 4 行 + 轨道列。
  - 形状升级为「真·原生矢量」：轨道线 = **line 形状 + tailEnd 箭头**（`prstGeom="line"` + `<a:tailEnd type="triangle"/>`）；参照线 = **line 形状 + prstDash=dash 虚线**；进度条 = roundRect 圆角矩形（白色粗体名称）；关键节点 = 按 symbol 映射 ellipse/triangle/star5/roundRect + 名称文本框。
- **验证** 导出核验：48 个 twoCellAnchor = 7 轨道箭头线 + 3 进度条 + 18 节点形状 + 18 节点名称框 + 2 虚线参照线，`<xdr:pic>`=0（全部原生形状）；drawing1.xml / sheet1.xml / Content_Types / rels 经 Python XML 解析器校验全良构；生产重启 forge 在线。
- **产出** 页面「导出 Excel」按钮即新版本；本地文件 `D:/tmp/项目规划_快速排期_月度示意_可视化.xlsx`。
